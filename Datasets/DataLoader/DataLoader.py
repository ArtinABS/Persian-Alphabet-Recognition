...
class DataLoader:
    def __init__(self, path='', image_size=50, shrink=0, padding=10, contrast=100, zoom=0.5, invert=False):
        self.PATH = path
    ...
    def normalize(self, image):
        """
        Normalize the image by ensuring the main content fits within a specific central box.
        
        Parameters:
            image (np.array): The input image to normalize.
        ...
        central_box_size = int(self.IMAGE_SIZE * self.ZOOM)
        central_box_start = (self.IMAGE_SIZE - central_box_size) // 2
        central_box_end = central_box_start + central_box_size

        if (x >= central_box_start and y >= central_box_start and 
            x + w <= central_box_end and y + h <= central_box_end):
            output_image = np.zeros_like(image)
            start_x = (output_image.shape[1] - w) // 2
            start_y = (output_image.shape[0] - h) // 2
            output_image[start_y:start_y + h, start_x:start_x + w] = image[y:y+h, x:x+w]
        else:
            scale_factor = min(central_box_size / w, central_box_size / h)
            resized_content = cv2.resize(image[y:y+h, x:x+w], (int(w * scale_factor), int(h * scale_factor)))

            output_image = np.zeros_like(image)
            start_x = (output_image.shape[1] - resized_content.shape[1]) // 2
            start_y = (output_image.shape[0] - resized_content.shape[0]) // 2
            output_image[start_y:start_y + resized_content.shape[0], start_x:start_x + resized_content.shape[1]] = resized_content
